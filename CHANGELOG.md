...
- **Global Concurrency Limiter:** Added new middleware to restrict maximum simultaneous inflight requests (`MAX_CONCURRENT_REQUESTS=20` default). Returns `429 Too Many Requests` with a descriptive message if the server is saturated.
...
- **Upstream Connection Pooling & Queuing:** Overhauled `upstream.service.js` with an asynchronous waiting queue (max 3 concurrent outbound requests) and global interval pacing. Queue depth is bounded (`UPSTREAM_QUEUE_MAX=50` default); excess requests are rejected immediately with `503 Service Unavailable` to prevent unbounded memory growth under sustained load. Protects upstream APIs from being blasted by parallel proxy requests.
