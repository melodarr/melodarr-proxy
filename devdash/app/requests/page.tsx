...
function statusClass(status: string) {
  ...
  if (status === "error" || status === "miss") return "text-red-300";
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-background/40 p-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-sm font-semibold text-gray-200">Raw trace</h3>
        <button
          type="button"
          onClick={copyRaw}
          className="inline-flex items-center gap-2 rounded-md border border-border/70 px-2.5 py-1 text-xs text-gray-300 hover:bg-white/5"
        >
          <Copy className="h-3.5 w-3.5" />
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
      <pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap rounded-md bg-black/30 p-3 text-xs leading-5 text-gray-300">
        {raw}
      </pre>
    </div>
  );
...
function TraceDetail({ trace }: { trace: Trace }) {
  ...
  return (
    <aside className="space-y-4 rounded-lg border border-border/60 bg-card p-5">
      <div>
        <h2 className="text-xl font-semibold">{trace.query}</h2>
        <p className="mt-1 break-all font-mono text-xs text-gray-500">{trace.id}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-md border border-border/50 bg-background/40 p-3">
          <div className="text-xs text-gray-500">Total time</div>
          <div className="mt-1 font-mono text-lg text-orange-300">{Math.round(trace.totalTime)}ms</div>
        </div>
        <div className="rounded-md border border-border/50 bg-background/40 p-3">
          <div className="text-xs text-gray-500">Cache</div>
          <div className={`mt-1 text-lg ${trace.cacheHit ? "text-emerald-300" : "text-gray-300"}`}>
      ...
      <div className="rounded-lg border border-border/60 bg-background/40 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {(trace.providersUsed ?? []).length === 0 ? (
            <span className="text-sm text-gray-500">No providers recorded.</span>
          ) : (
      ...
      <div className="rounded-lg border border-border/60 bg-background/40 p-4">
        ...
        <div className="mt-4 space-y-3">
          {steps.length === 0 ? (
            <div className="text-sm text-gray-500">No step timings recorded.</div>
          ) : (
            steps.map((step) => {
              ...
              return (
                <div key={`${step.name}-${step.duration}-${step.status}`}>
                  <div className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="font-mono text-xs text-gray-400">{step.duration ?? 0}ms</span>
                  </div>
      ...
      <div className="rounded-lg border border-border/60 bg-background/40 p-4 text-sm">
        ...
        <dl className="mt-3 space-y-2 text-xs">
          <div className="flex justify-between gap-4">
            ...
            <dd className="text-right text-gray-300">{formatTimestamp(trace.timestamp)}</dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="text-gray-500">Start time</dt>
            <dd className="text-right font-mono text-gray-300">{trace.startTime ?? "--"}</dd>
          </div>
        </dl>
      </div>
  ...
  return (
    <main className="container mx-auto max-w-screen-2xl space-y-8 p-8">
      <div>
        ...
        <p className="mt-2 text-gray-400">Recent proxy traces, provider calls, cache behavior, and execution timing.</p>
      </div>

      {error && <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-300">{error.message}</div>}
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_520px]">
        <div className="rounded-lg border border-border/60 bg-card">
          ...
          {isLoading ? (
            <div className="p-8 text-sm text-gray-500">Loading traces...</div>
          ) : traces.length === 0 ? (
            <div className="p-8 text-sm text-gray-500">No requests have been recorded yet.</div>
          ) : (
              ...
              {traces.map((trace) => {
                ...
                return (
                  <button
                    ...
                    className={`grid w-full grid-cols-1 gap-3 p-5 text-left transition-colors hover:bg-white/5 md:grid-cols-[1fr_120px_180px] ${
                      selected ? "bg-blue-500/10" : ""
                    }`}
                  >
                    <div>
                      <div className="font-medium">{trace.query}</div>
                      <div className="mt-1 font-mono text-xs text-gray-500">{trace.id}</div>
                    ...
                    <div className="flex items-center gap-2 font-mono text-sm text-orange-300">
                      <Clock className="h-4 w-4" />
                      {Math.round(trace.totalTime)}ms
                    </div>
                    <div className="text-sm text-gray-500">{formatTimestamp(trace.timestamp)}</div>
                  </button>
                );
              })}
            </div>
        ...
        {selectedTrace ? (
          ...
          <div className="rounded-lg border border-border/60 bg-card p-8 text-sm text-gray-500">Select a trace to view details.</div>
        )}
      </section>
    </main>
  );
