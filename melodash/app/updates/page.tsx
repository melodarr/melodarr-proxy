...
function Changelog({ body }: { body?: string }) {
 if (!body) {
 return <p className="text-sm text-muted">No changelog was provided for this release.</p>;
 }

 return (
 <pre className="whitespace-pre-wrap rounded-lg border border-border bg-page p-4 text-sm leading-6 text-secondary ">
 {body}
 </pre>
 );
}
...
export default function UpdatesPage() {
 ...
 <p className="mt-2 text-secondary">Review releases, read the changelog, and apply updates from Melodash.</p>
 </div>
 <button
 type="button"
 onClick={() => mutate()}
 className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-2 text-sm text-primary transition-colors hover:bg-black/5 dark:hover:bg-black/5 dark:bg-black/5 dark:bg-card/5"
 >
 ...
 <p className="text-sm text-secondary">Installed</p>
 <p className="mt-2 text-3xl font-semibold">v{data?.currentVersion ?? "--"}</p>
 </div>
 <div className="rounded-lg border border-border bg-card p-6">
 <p className="text-sm text-secondary">Latest release</p>
 <p className="mt-2 text-3xl font-semibold">v{data?.latest?.version ?? "--"}</p>
 <p className="mt-2 text-xs text-muted">{formatDate(data?.latest?.publishedAt)}</p>
 </div>
 <div className="rounded-lg border border-border bg-card p-6">
 ...
 <p className="mt-3 text-xs text-muted">Checked {formatDate(data?.checkedAt)}</p>
 </div>
 </section>
 ...
 <p className="mt-1 text-sm text-secondary">{data?.latest?.name || data?.latest?.tagName || "Latest release"}</p>
 </div>
 {data?.latest?.htmlUrl && (
 <a
 ...
 target="_blank"
 rel="noreferrer"
 className="inline-flex items-center gap-2 rounded-md border border-border px-4 py-2 text-sm text-primary hover:bg-black/5 dark:hover:bg-black/5 dark:bg-black/5 dark:bg-card/5"
 >
 Open release
 ...
 className="inline-flex items-center justify-center gap-2 rounded-md bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500 disabled:cursor-not-allowed disabled:bg-gray-700 disabled:text-secondary"
 >
 ...
 <pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap rounded-md bg-page p-3 text-xs text-secondary ">
