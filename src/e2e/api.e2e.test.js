...
describe('API E2E Tests', () => {
  ...
  describe('Core Endpoints', () => {
    ...
    it('GET /api/artist/{mbid} → returns Lidarr add refetch metadata lists', async () => {
      ...
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'artistId'), true)
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'artists'), true)
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'releases'), true)
      assert.ok(Array.isArray(album.releases))
      assert.ok(album.releases.length > 0, 'artist metadata must include nested album releases')
      assert.ok(Array.isArray(album.releases[0].tracks))
      assert.ok(album.releases[0].tracks.length > 0, 'artist metadata must include nested release tracks')
    })
    ...
    it('Lidarr POST /api/v1/artist add flow can refetch SkyHook metadata by foreignArtistId', async () => {
      ...
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'artists'), true)
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'releases'), true)
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'images'), true)
      assert.strictEqual(Object.prototype.hasOwnProperty.call(album, 'links'), true)
      assert.ok(Array.isArray(album.releases))
      assert.ok(album.releases.length > 0, 'metadata refetch should include nested album releases')
      assert.ok(Array.isArray(album.releases[0].tracks))
      assert.ok(album.releases[0].tracks.length > 0, 'metadata refetch should include nested release tracks')
    })
  })
